...
namespace OpenRA.Mods.Common.Widgets
{
	public class ObserverBuildOrderIconsWidget : Widget
	{
		...
		readonly CachedTransform<Player, BuildOrderTracker> tracker = new(player => player.PlayerActor.TraitOrDefault<BuildOrderTracker>());

		IEnumerable<ArmyUnit> buildOrder;
		int lastCount;
		...
		public override void Draw()
		{
			...
			var buildOrderTracker = tracker.Update(player);

			if (lastCount != buildOrderTracker.Count)
			{
				buildOrder = UpdateBuildOrder(buildOrderTracker, player);
			}
		...
		IEnumerable<ArmyUnit> UpdateBuildOrder(BuildOrderTracker buildOrderTracker, Player player)
		{
			lastCount = buildOrderTracker.Count;
			return buildOrderTracker.BuildOrder.Select(u => new ArmyUnit(world.Map.Rules.Actors[u], player));
		}
