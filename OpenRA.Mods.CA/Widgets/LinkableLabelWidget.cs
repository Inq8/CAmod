...
namespace OpenRA.Mods.CA.Widgets
{
	...
	public class LinkableLabelWidget : LabelWidget
	{
		...
		public string HoverCursor = "press";
			...
			foreach (var line in lines)
			{
				...
				while (charIndex < line.Length && segmentIndex < segments.Count)
				{
					...
					if (isLink)
					{
						...
						// Expand clickable area vertically for easier clicking
						const int verticalPadding = 3;
						linkRegions.Add(new LinkRegion
						{
							Bounds = new Rectangle(lineX, lineY - verticalPadding, segmentWidth, segmentHeight + verticalPadding * 2),
		...
		{
			return hoveredLink != null ? HoverCursor : base.GetCursor(pos);
		}
