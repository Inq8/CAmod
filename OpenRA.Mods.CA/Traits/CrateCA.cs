...
{
	[Desc("Copy of base version with a crash fix for when location is set before crate is added to world.")]
	public class CrateCAInfo : TraitInfo, IPositionableInfo, Requires<RenderSpritesInfo>
	{
		...
		public override object Create(ActorInitializer init) { return new CrateCA(init, this); }
	...
	public class CrateCA : ITick, IPositionable, ICrushable, ISync, INotifyCreated,
		INotifyParachute, INotifyAddedToWorld, INotifyRemovedFromWorld, INotifyCrushed
	{
		readonly Actor self;
		readonly CrateCAInfo info;
		bool collected;
		...
		public CrateCA(ActorInitializer init, CrateCAInfo info)
		{
			self = init.Self;
			this.info = info;
